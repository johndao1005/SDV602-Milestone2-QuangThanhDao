...
class window(Tk):
    ...
    def DES_window(self,windowname, datatype, next, prev,positionX ,positionY):
        ...
        windowname.geometry(f"940x740+{positionX}+{positionY}")
        self.DES_check.add(datatype)
        # ANCHOR data display
        
        chart_create.draw_graph(windowname,datatype)
        # Storing new data
        input = StringVar()
        chatLog = StringVar()
        # chat box creating and function
        frame1 = ttk.LabelFrame(windowname,text="Chat box",borderwidth=0)
        frame1.grid(column=2,row=1,**option1,sticky = E)
        chatBox = ttk.Label(frame1,
                        background='light gray',
                        textvariable=chatLog,width = 40
                        ).grid(column=1,row=1,**option1,columnspan=2)
        entry = ttk.Entry(frame1, textvariable=input).grid(column=1,row=2,**option1,sticky = E)
        button = ttk.Button(frame1,
                        text="Send",
                        command=lambda: chat()
                        ).grid(column=2,row=2,**option1,sticky = E)
        ...
        button = ttk.Button(windowname,
                        ...
                        ).grid(column=2,row=6,**option1,sticky=SE)
        def chat():
